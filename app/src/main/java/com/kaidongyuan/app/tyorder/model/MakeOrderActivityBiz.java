...
public class MakeOrderActivityBiz {
     ...
     * @param productIDX 需要查找的产品IDX
     * @return
     */
    public Product searchProduct(List<Product> productlist,Long productIDX){

        for (int i=0;i<productlist.size();i++){
            if (productIDX==productlist.get(i).getIDX()){
     ...
     */
    public void addProductSize(Long dataIndex) {
        try {
            Product choiceProduct =searchProduct(mCurrentProductData,dataIndex);
    ...
    public void addProductSizeInChoicedProducts(int dataIndex) {
        try {
            ...
            float size = FloatArithUtil.add(product.getCHOICED_SIZE(),1) ;
     ...
     * @param dataIndex 在当前选择品类商品中的位置
     */
    public void deleteProductSize(Long dataIndex) {
        try {
            Product choiceProduct =searchProduct(mCurrentProductData,dataIndex);
            int index = getChoiceProductIndexInChoicedProductList(choiceProduct);
    ...
    public void deleteProductSizeInChoicedProducts(int index) {
        try {
            if (index != -1) {
                ...
                float choiceSize = product.getCHOICED_SIZE();
                if (choiceSize >= 1) {
                    product.setCHOICED_SIZE(FloatArithUtil.sub(choiceSize,1));
                }else {
                    product.setCHOICED_SIZE(0f);
     ...
     */
    public void setProductSize(Long dataIndex, float productCount) {
        try {
            Product choiceProdut = searchProduct(mCurrentProductData,dataIndex);
     ...
     */
    public void setProductSizeInChoicedProducts(int index, float productCount) {
        try {
            Product product = mChoiceProducts.get(index);
            if (BusinessConstants.ISNEED_CARE_NVENTORY.equals(product.getISINVENTORY())) {
                int maxSize = product.getPRODUCT_INVENTORY();
    ...
    public String getChoicedProductSize() {
        try {
            float choicedProductSize = 0;
            for (Product product : mChoiceProducts) {
                choicedProductSize=FloatArithUtil.add(choicedProductSize,product.getCHOICED_SIZE());
            }
    ...
    public String getChoicedProductPrice() {
        try {
            double totalPrice = 0;
            for (Product product : mChoiceProducts) {
                double tempcoust=DoubleArithUtil.mul( product.getPRODUCT_PRICE(),product.getCHOICED_SIZE());
                totalPrice =DoubleArithUtil.add(totalPrice,tempcoust);
            }
     ...
     * @param msg 输入的信息
     */
    public List<Product> searchParty(String msg) {
        ArrayList<Product> searchProducts = new ArrayList<>();
        try {
            if (msg == null || msg.trim().length() <= 0) {
                return getmCurrentProductData();
            } else {

                for (Product product : getmCurrentProductData()) {
