...
import com.kaidongyuan.app.tyorder.adapter.ChoicedProductAdapter;
import com.kaidongyuan.app.tyorder.adapter.OrderBrandsAdapter;
import com.kaidongyuan.app.tyorder.adapter.OrderProductAdapter;
import com.kaidongyuan.app.tyorder.adapter.OrderTypesAdapter;
import com.kaidongyuan.app.tyorder.adapter.PaymentTypeAdapter;
...
import com.kaidongyuan.app.tyorder.model.MakeOrderActivityBiz;
...
public class MakeOrderActivity extends BaseActivity implements View.OnClickListener, SwipeRefreshLayout.OnRefreshListener, AdapterView.OnItemClickListener {
    ...
    private void setListener() {
        try {
            ...
            mOrderProductAdapter.setInterface(new OrderProductAdapter.OrderProductAdapterInterface() {
                @Override
                public void addProduct(Long dataIndex) {
                    mBiz.addProductSize(dataIndex);
                    notifyChoicedProductsChange();
                }

                @Override
                public void deleteProduct(Long dataIndex) {
                    mBiz.deleteProductSize(dataIndex);
                    notifyChoicedProductsChange();
                }

                @Override
                public void setProductCount(Long dataIndex, float productCount) {
                    mBiz.setProductSize(dataIndex, productCount);
                    notifyChoicedProductsChange();
                }
            });

            mOrderProductAdapter.setSearchInterface(new OrderProductAdapter.SearchProductInterface() {
                @Override
                ...
                public void afterTextChanged(Editable s) {
                    try {
                        ...
                        List<Product> searchProducts=mBiz.searchParty(msg);//搜索客户
                        searchDataChange(searchProducts);
                    } catch (Exception e) {
                        ExceptionUtil.handlerException(e);
                    }
                }
            ...
            mChoicedProductAdapter.setInterface(new ChoicedProductAdapter.choicedProductAdapterInterface() {
                ...
                @Override
                public void setProductCount(int dataIndex, float giftCount) {
     ...
     */
    private void searchDataChange(List<Product> products) {
        try {
            mDrawerLayout.closeDrawers();
     ...
     * 数据发生变化，刷新每个 ListView 中的数据
     */
    private void notifyChoicedProductsChange() {
        try {
            mDrawerLayout.closeDrawers();
            mChoicedProductAdapter.notifyChange(mBiz.getChoiceProducts());
            mOrderBrandsAdapter.notifyChange(mBiz.getOrderBrands(), mBiz.getCurrentSelectedBrandIndex());
            mOrderTypesAdapter.notifyChange(mBiz.getOrderTypes(), mBiz.getCurrentSelectedOrderTypeIndex());
            //  mOrderProductAdapter.notifyChange(mBiz.getmCurrentProductData(), mBiz.getChoiceProducts());
            mOrderProductAdapter.notifyChoicedProductsChange( mBiz.getChoiceProducts());
            mTextViewCurrentOrderType.setText(String.valueOf(mBiz.getCurrentProductType()));
            mTtextViewCurrentOrderBrand.setText(String.valueOf(mBiz.getCurrentOrderBrand()));
            mTextViewChoicedProductSize.setText(String.valueOf(mBiz.getChoicedProductSize()));
            mTextViewChoicedProductPrice.setText(String.valueOf(mBiz.getChoicedProductPrice()));
