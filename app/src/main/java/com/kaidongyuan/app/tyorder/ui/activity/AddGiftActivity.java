...
import com.kaidongyuan.app.tyorder.adapter.OrderCategoryAdapter;
import com.kaidongyuan.app.tyorder.adapter.OrderCommodityAdapter;
import com.kaidongyuan.app.tyorder.adapter.OrderGiftChoiceDetialAdapter;
...
import com.kaidongyuan.app.tyorder.util.ChangeProductToPromotionDetailUtil;
...
public class AddGiftActivity extends BaseActivity implements View.OnClickListener {
    ...
    private class InnerCommodityAdapterInterface implements OrderCommodityAdapter.CommodityAdapterInterface {
        ...
        @Override
        public void setGiftCount(int dataIndex, float giftCount) {
            if (giftCount < 0) {
    ...
    private void addGiftToGiftDetial(PromotionDetail promotionDetail) {
        try {
            ...
            if (giftIndex != -1) {
                mChoiceGiftData.get(giftIndex).PO_QTY = FloatArithUtil.add(mChoiceGiftData.get(giftIndex).PO_QTY,1);
            } else {
                promotionDetail.PO_QTY = 1;
                mChoiceGiftData.add(promotionDetail);
            }
            mCategoryData.get(index).setChoiceCount(DoubleArithUtil.add(mCategoryData.get(index).getChoiceCount(),1) );
            if (NEED_CARE_STOCK.equals(promotionDetail.LOTTABLE09)) {//是否需要考虑库存
                promotionDetail.LOTTABLE11=FloatArithUtil.sub(promotionDetail.LOTTABLE11,1);
            }
            notifyGiftChoiceChanged(mCategoryIndex);
        } catch (Exception e) {
            ExceptionUtil.handlerException(e);
    ...
    private void deleteGiftFromGiftDetial(PromotionDetail promotionDetail) {
        try {
            ...
            int giftIndex = mChoiceGiftData.indexOf(promotionDetail);
            if (giftIndex != -1) {
                float giftCount = mChoiceGiftData.get(giftIndex).PO_QTY;
                if (giftCount >=1) {
                    mChoiceGiftData.get(giftIndex).PO_QTY = FloatArithUtil.sub(mChoiceGiftData.get(giftIndex).PO_QTY,1);
                    int index = getPromotionDetailInCategoryIndex(promotionDetail);
                    mCategoryData.get(index).setChoiceCount(mCategoryData.get(index).getChoiceCount() - 1);
                }else {
                    mChoiceGiftData.get(giftIndex).PO_QTY=0f;
                    mChoiceGiftData.remove(giftIndex);
                }
                if (NEED_CARE_STOCK.equals(promotionDetail.LOTTABLE09)) {
                    promotionDetail.LOTTABLE11=FloatArithUtil.add(promotionDetail.LOTTABLE11,1);
                }
            }
            notifyGiftChoiceChanged(mCategoryIndex);
        } catch (Exception e) {
     ...
     */
    private void setGiftCountInGiftDetial(PromotionDetail promotionDetail, float giftCount) {
        try {
            if (giftCount < 0) {
                Toast.makeText(AddGiftActivity.this, "请输入大于零的数量！", Toast.LENGTH_SHORT).show();
                return;
