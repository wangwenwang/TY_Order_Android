 ...
 * 在途订单界面的业务类
 */
public class CheckOrderFragmentUnAduitOrderPagerBiz {

    private CheckOrderFragmentUnAduitOrderPage mPager;
    ...
    public CheckOrderFragmentUnAduitOrderPagerBiz(CheckOrderFragmentUnAduitOrderPage page) {
        try {
            this.mPager = page;
        ...
        try {
            StringRequest request = new StringRequest(Request.Method.POST, URLCostant.GET_ORDER_LIST, new Response.Listener<String>() {
                ...
                public void onResponse(String response) {
                    Logger.w(CheckOrderFragmentUnAduitOrderPagerBiz.this.getClass() + ".getOrderDataSuccess:" + response);
                    getOrderDataSuccess(response);
                }
            }, new Response.ErrorListener() {
                @Override
                public void onErrorResponse(VolleyError error) {
                    Logger.w(CheckOrderFragmentUnAduitOrderPagerBiz.this.getClass() + ".getOrderDataError:" + error.toString());
        ...
        try {
            ...
            if (type == 1) {
                if (mPageIndex == mInitPagerIndex) {//刷新或初次加载，清除集合中的数据
                    mPager.getmOrderListApapter().refreshCheckedOrderIDXs();
                    mOrderList.clear();
                }
                ...
                if (object.getString("msg").equals("没有数据")&&mPageIndex==mInitPagerIndex){
                    mOrderList.clear();
                    mPager.loginSuccess();
                }else {
